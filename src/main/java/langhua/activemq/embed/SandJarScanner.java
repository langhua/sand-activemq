...
public class SandJarScanner implements JarScanner {
    private static final String module = SandJarScanner.class.getName();
    ...
    static {
        Debug.logVerbose("============== SandJarScanner ==============", module);
        Set<ClassLoader> cls = new HashSet<>();

        ClassLoader cl = SandJarScanner.class.getClassLoader();
        while (cl != null) {
            cls.add(cl);
            cl = cl.getParent();
        }
     ...
     */
    private JarScanFilter jarScanFilter = new SandJarScanFilter();
    ...
    public void scan(JarScanType scanType, ServletContext context, JarScannerCallback callback) {
        ...
        Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.webinflibStart"), module);
        ...
        if (dirList != null) {
            for (String path : dirList) {
                if (path.endsWith(Constants.JAR_EXT)
                    ...
                    Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.webinflibJarScan", path), module);
                    URL url = null;
                    try {
                        ...
                        Debug.logError(sm.getString("jarScan.webinflibFail", url) + e, module);
                    }
                } else {
                    Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.webinflibJarNoScan", path), module);
                }
            }
        }
                ...
                if (isScanAllDirectories()) {
                    ...
                    if (url != null) {
                        try {
                            ...
                            Debug.logWarning(sm.getString("jarScan.webinfclassesFail") + e, module);
                        }
                    }
                }
            }
    ...
    protected void doScanClassPath(JarScanType scanType, ServletContext context, JarScannerCallback callback,
        ...
        Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.classloaderStart"), module);
    ...
    protected void processURLs(JarScanType scanType, JarScannerCallback callback, Set<URL> processedURLs,
        ...
        while (!classPathUrlsToProcess.isEmpty()) {
            ...
            if ((cpe.isJar() || scanType == JarScanType.PLUGGABILITY || isScanAllDirectories())
                ...
                Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.classloaderJarScan", url), module);
                try {
                    processedURLs.add(url);
                    process(scanType, callback, url, null, isWebapp, classPathUrlsToProcess);
                } catch (IOException ioe) {
                    Debug.logError(sm.getString("jarScan.classloaderFail", url) + ioe, module);
                }
            } else {
                // JAR / directory has been skipped
                Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.classloaderJarNoScan", url), module);
            }
        }
    }
    ...
    protected void addClassPath(Deque<URL> classPathUrlsToProcess) {
        ...
        for (String classPathEntry : classPathEntries) {
            ...
            try {
                ...
                Debug.logError(sm.getString("jarScan.classPath.badEntry", classPathEntry) + e, module);
            }
        }
    }
    ...
    protected void process(JarScanType scanType, JarScannerCallback callback, URL url, String webappPath,
        ...
        Debug.logVerbose("[" + scanType + "] " + sm.getString("jarScan.jarUrlStart", url), module);
    ...
    private static void processManifest(Jar jar, boolean isWebapp, Deque<URL> classPathUrlsToProcess)
        ...
        if (manifest != null) {
            ...
            Debug.logVerbose("--Scanning manifest of " + jarURL.getPath() + "--", module);
            ...
            for (String classPathEntry : classPathEntries) {
                ...
                if (classPathEntry.startsWith("\\")) {
                    classPathEntry = classPathEntry.substring(1);
                    if (classPathEntry.endsWith(Constants.JAR_EXT)) {
                        classPathEntry = "file:/" + classPathEntry.replaceAll("\\\\", "/");
                    }
                }
                Debug.logVerbose("-- Manifest class path --" + classPathEntry, module);
                ...
                try {
                    ...
                    Debug.logError(sm.getString("jarScan.invalidUri", jarURL) + e, module);
                    continue;
                }
